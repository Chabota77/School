...
app.get('/api/teachers/:id', authenticateToken, (req, res) => {
    ...
    db.query(query, [id], (err, results) => {
        ...
        // Normalize response (handle multiple assignments if we supported them, but currently 1-to-1 mostly)
        const teacher = {
            id: results[0].id,
            name: results[0].name,
            ...
            classIds: results.filter(r => r.class_id).map(r => r.class_id),
            subjectIds: results.filter(r => r.subject_id).map(r => r.subject_id),
...
app.post('/api/students', authenticateToken, (req, res) => {
    const { name, age, gender, class_id, status, password } = req.body;
    ...
    db.query(checkQuery, [name, class_id || null], (err, existing) => {
        ...
        // Determine password (custom or default 'student123')
        const plainPassword = password && password.trim() !== '' ? password : 'student123';
        const hashedPassword = bcrypt.hashSync(plainPassword, 10);

        const query = 'INSERT INTO students (name, age, gender, class_id, status, password) VALUES (?, ?, ?, ?, ?, ?)';
        db.query(query, [name, age, gender, class_id, status || 'Enrolled', hashedPassword], (err, results) => {
            if (err) return res.status(500).json({ error: err.message });
